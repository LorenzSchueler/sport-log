...
// DateTime cannot contain the timezone in `+00:00` format because `+` is not allowed as a HTTP
// query string character. Therefor the datetime must be converted like so:
