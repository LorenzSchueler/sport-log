...
pub async fn adm_get_platforms(
    ...
    match id {
        Some(id) => {
            ...
            Platform::get_by_id(platform_id, &db).map(|p| vec![p])
        }
        None => Platform::get_all(&db),
    }
    .map(Json)
    .map_err(Into::into)
}
...
pub async fn adm_update_platforms(
    ...
    match platforms {
        UnverifiedSingleOrVec::Single(platform) => {
            let platform = platform.verify_adm(auth)?;
            Platform::update(platform, &db)
        }
        UnverifiedSingleOrVec::Vec(platforms) => {
            let platforms = platforms.verify_adm(auth)?;
            Platform::update_multiple(platforms, &db)
        }
    }
    .map(|_| StatusCode::OK)
    .map_err(Into::into)
...
pub async fn create_platform_credentials(
    ...
    match platform_credentials {
        UnverifiedSingleOrVec::Single(platform_credential) => {
            ...
            PlatformCredential::create(platform_credential, &db)
        }
        UnverifiedSingleOrVec::Vec(platform_credentials) => {
            ...
            PlatformCredential::create_multiple(platform_credentials, &db)
        }
    }
    .map(|_| StatusCode::OK)
    .map_err(Into::into)
...
pub async fn get_platform_credentials(
    ...
    match id {
        Some(id) => {
            let platform_id = id.verify_user(auth, &db)?;
            PlatformCredential::get_by_id(platform_id, &db).map(|p| vec![p])
        }
        None => PlatformCredential::get_by_user(*auth, &db),
    }
    .map(Json)
    .map_err(Into::into)
}
...
pub async fn update_platform_credentials(
    ...
    match platform_credentials {
        UnverifiedSingleOrVec::Single(platform_credential) => {
            let platform_credential = platform_credential.verify_user(auth, &db)?;
            PlatformCredential::update(platform_credential, &db)
        }
        UnverifiedSingleOrVec::Vec(platform_credentials) => {
            let platform_credentials = platform_credentials.verify_user(auth, &db)?;
            PlatformCredential::update_multiple(platform_credentials, &db)
        }
    }
    .map(|_| StatusCode::OK)
    .map_err(Into::into)
